...
public class ReservasView extends JFrame {
	...
	private void guardarReserva() {
		...
		reservaControl.guardar(nuevaReserva);
	...
	private void calcularValor(JDateChooser fechaE, JDateChooser fechaS) {
		if(fechaE.getDate() != null && fechaS.getDate() != null) {
			Calendar inicio = fechaE.getCalendar();//Obtenemos el dia de inicio
			Calendar fin = fechaS.getCalendar();//Obtenemos el dia de fin
			int dias = -1;//-1 para contar a partir del siguiente dia
			int cuotaD = 1000;
			int valor;
			
			while(inicio.before(fin)|| inicio.equals(fin)) {
				dias++;
				inicio.add(Calendar.DATE, 1);//1 es el dia a ser aumentado
			}
			valor = dias * cuotaD;
